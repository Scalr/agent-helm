...
- Added optional entrypoint overrides for the controller and worker containers (`agent.controller.command`, `agent.controller.args`, `task.worker.command`, `task.worker.args`). These default to empty, so both containers use the image's built-in `ENTRYPOINT`/`CMD`, which replaces the previously hard-coded `command: ["python", "-m", "tacoagent.cmd"]` in the chart. Using the image default is recommended; overriding the entrypoint bypasses the image's tuned startup and may break the container or degrade its performance, so the override should be left empty in normal operation.
