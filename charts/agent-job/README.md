...
The controller and worker containers run the Scalr Agent service from the [`scalr/agent`](https://hub.docker.com/r/scalr/agent) image. The runner container — where Terraform/OpenTofu, OPA, and shell hooks actually execute — uses a separate image (see [Custom Runner Image](#custom-runner-image)).

If you need to bake extra software (custom certificate bundles, internal CAs, sidecar binaries used by the controller, etc.) into the agent container, build your own image on top of `scalr/agent`. Follow the contract from the [Scalr Build Custom Agent Image guide](https://docs.scalr.io/docs/run-environment#build-custom-agent-image):
...
Then point the chart at the resulting image (`agent.image` is shared between the controller and worker containers):
...
For tooling that the **run itself** needs (cloud CLIs, language runtimes, custom scripts), customize the runner image instead — see below.
