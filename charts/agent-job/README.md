...
Alternatively, for workloads where a shared network volume becomes the bottleneck, the cache can be backed by a node-local disk (local SSD) via `persistence.cache.hostPath` — a local disk is faster than any network volume, at the cost of a per-node cache scope. When `hostPath.enabled` is true, it takes precedence over `persistentVolumeClaim` and `emptyDir`, and no PVC is created. The node path must be prepared (made writable by the agent user) by a cluster-wide node-preparation DaemonSet — see the [hostPath guide](docs/cache-persistence-hostpath.md).
...
persistence:
  cache:
    hostPath:
      enabled: true
      # Node-local disk path; for GKE node pools with local SSDs,
      # use a subdirectory of /mnt/disks/ssd0
