...
```mermaid
flowchart LR
    A[Agent controller] -- OTLP gRPC --> C[OTel Collector]
    B[Agent worker Job] -- OTLP gRPC --> C
    C -- vendor protocol --> D[Observability Backend]
```

The endpoint is a `host:port` reachable from both the controller Pod and the task worker Pods. Common values: `otel-collector:4317` (Service in the same namespace), `otel-collector.<ns>.svc.cluster.local:4317` (cross-namespace Service), or an external/vendor gateway.
