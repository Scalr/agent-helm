...
| agent.container_task_image | string | `""` | The OCI image configuration for the golden runner image. Enables the use of a golden runner image or allows changing the image if golden runner images are already enabled at the Scalr account level. When enabled, a single runner image will be used instead of software-specific Docker images (e.g., scalr/opentofu:x.y.z, where x.y.z depends on workspace settings). OpenTofu/Terraform/etc. will be shipped as binary releases from scalr.io rather than individual Docker images. Note that not all software versions are available as binary releases on the Scalr platform, and some older OPA/Checkov versions may not be available. For newer accounts, golden runner image is enabled and can be overridden by this option. For older accounts, golden runner image can be enforced using this option. Alternatively, you can contact the Scalr Team to enable it for all agents in your account by default. The default image used by the Scalr platform is the runner image: https://hub.docker.com/r/scalr/runner |
| agent.container_task_image_registry | string | `""` | The OCI image registry configuration for deprecated software-specific Docker images. Enforce the use of a custom image registry to pull all container task images. This option only applies to software-specific Docker images (e.g., scalr/opentofu:x.y.z or scalr/terraform:x.y.z) and is not applied to the golden runner image, which can be configured via containerTaskImage. If the registry ends with a trailing slash, it will be appended to the original namespace/image-name. Otherwise, the namespace will be erased. Examples:   - gcr.io/my/org/ + scalr/runner:0.0.1 => gcr.io/my/org/scalr/runner:0.0.1   - gcr.io/my/org + scalr/runner:0.0.1 => gcr.io/my/org/runner:0.0.1 |
