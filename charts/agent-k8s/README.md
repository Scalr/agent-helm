...
| agent.container_task_image | string | `""` | Enforce the use of a custom runner image instead of the default scalr/runner:x.y.z. Use this to pin a specific version or provide a custom image. Only applies when `agent.container_task_image_enabled` is true. This option overrides container_task_image_registry. Example: 'scalr/runner:1.2.3' or 'myregistry.io/custom-runner:latest' |
| agent.container_task_image_enabled | bool | `false` | Enable golden runner image mode. When enabled, uses a single runner image (scalr/runner) instead of version-specific Docker images (e.g., scalr/opentofu:x.y.z). IaC tools are shipped as binary releases from scalr.io rather than individual Docker images. Note: Not all software versions are available as binary releases; some older OPA/Checkov versions may not be available. |
| agent.container_task_image_registry | string | `""` | Enforce the use of a custom image registry to pull all container task images. All images must be preemptively pushed to this registry for the agent to work. The registry path may include a repository prefix to be replaced. If the path ends with a trailing slash, it will be appended to the original repository. Only applies to Docker-based releases when `agent.container_task_image_enabled` is false. Examples: 'mirror.io', 'mirror.io/myproject', or 'mirror.io/myproject/' |
