...
- Enable [single execution mode](https://docs.scalr.io/docs/configuration#scalr_agent_single) to restart the container after each execution and disable provider/module caching:

```yaml
extraEnv:
  # reinitialize container after each run
...
- Use separate agent node pools for each RBAC perimeter (e.g., Scalr Environment). For example, you can ensure each team in your organization works on individual agent pools without having access to each other's resources.

If you want to manage a single node pool across different teams within your organization using a single agent deployment, consider using the [`agent-job`](/charts/agent-job) chart.
